...
const genreAccentClasses: Record<string, { bg: string; text: string; glow: string; border: string }> = {
  ...
  jazz: {
    ...
    glow: "shadow-[0_0_12px_hsl(var(--genre-jazz)/0.3)]",
  ...
  smooth: {
    ...
    glow: "shadow-[0_0_12px_hsl(var(--genre-smooth-rap)/0.3)]",
...
export const GlobalMiniPlayer = () => {
  ...
  const handleGenreSelect = (g: (typeof QUICK_GENRES)[number]) => {
    setPlayback(g.spotifyId, "mini-player", "playlist");
    openDrawer();
    setIsExpanded(false);
  };
      ...
      >
        ...
        <AnimatePresence>
          {(isExpanded || isHovering) && (
            ...
            >
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider px-2 mb-1.5">Quick Switch</p>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {/* Expand indicator */}
            <motion.div animate={{ rotate: isExpanded || isHovering ? 180 : 0 }} transition={{ duration: 0.2 }}>
          ...
          {/* Subtle pulse ring */}
          <motion.div
            className={cn("absolute inset-0 rounded-2xl border-2", accent.text, "opacity-20 pointer-events-none")}
            animate={{
              scale: [1, 1.05, 1],
              opacity: [0.2, 0.1, 0.2],
            }}
