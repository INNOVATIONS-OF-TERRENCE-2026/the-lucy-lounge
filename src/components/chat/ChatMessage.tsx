...
export function ChatMessage({ message, isStreaming }: ChatMessageProps) {
  ...
  const loadAttachments = async () => {
    ...
    const { data } = await supabase.from("attachments").select("*").eq("message_id", message.id);
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"} mb-4`} data-theme-area="chat">
      {!isUser && <LucyLogo size="sm" showGlow className="flex-shrink-0" />}
      ...
      <div className={`max-w-[85%] md:max-w-[75%] ${isUser ? "order-first" : ""}`}>
            ...
            <div className="prose prose-sm max-w-none dark:prose-invert prose-p:my-2 prose-p:leading-relaxed prose-headings:mt-4 prose-headings:mb-2 prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5 text-inherit">
              <ReactMarkdown
                components={{
                  ...
                  h3: ({ children }) => <h3 className="text-base font-bold mt-5 mb-2">{children}</h3>,
        ...
        {!isStreaming && message.id && userId && (
          <MessageReactions messageId={message.id} userId={userId} className="mt-2 px-1" />
        )}

        {!isUser && !isStreaming && (
          <p className="text-xs text-muted-foreground mt-2 px-1">{new Date(message.created_at).toLocaleTimeString()}</p>
        )}
      </div>
