/* 
  ...
  This is the orchestration brain of Lucy Chat.
*/
...
export function ChatInterface({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (imageIntent.isImage) {
        const image = await generateLucyImage(
          imageIntent.prompt,
        );
  ...
  return (
    <main className="flex-1 flex flex-col h-screen overflow-hidden">
      <ReadingProgressBar
        isStreaming={!!streamingMessage || isLocalStreaming}
      />
      ...
      >
        <div
          className={`max-w-5xl mx-auto ${getSpacingClass()}`}
        >
          {messages.map((msg, i) => (
