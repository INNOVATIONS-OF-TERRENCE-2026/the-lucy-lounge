...
export function ChatInterface({ userId, conversationId, onConversationCreated }: ChatInterfaceProps) {
  ...
  const processStreamingResponse = async (response: Response, convId: string, userContentForContext: string) => {
    ...
    if (fullResponse) {
      ...
      if (
        ...
        (fullResponse.toLowerCase().includes("remember") || fullResponse.toLowerCase().includes("important"))
      ) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!convId) {
        convId = await createConversation(userMessage || "New Conversation");
    ...
    <main className="flex-1 flex flex-col h-screen relative overflow-hidden" data-theme-area="chat">
      ...
      <header className="h-14 md:h-16 flex items-center justify-between px-4 md:px-6 backdrop-blur-md bg-background/60 border-b border-border/20 flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          <LoungeSwitcher /> {/* ✅ PERFECT SAFE INSERTION */}
          {isAdmin && (
            <Button
              variant="ghost"
              size="sm"
