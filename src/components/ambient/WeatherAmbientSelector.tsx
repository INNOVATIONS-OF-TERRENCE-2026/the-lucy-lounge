...
export const WeatherAmbientSelector = () => {
  ...
  const { weather, season, intensity, enabled, setWeather, setSeason, setIntensity, setEnabled } = useWeatherAmbient();
      ...
      <CollapsibleTrigger asChild>
        <Button variant="ghost" className="w-full justify-between px-3 py-2 h-auto hover:bg-accent/50">
          <div className="flex items-center gap-2">
            {activeWeatherOption && <activeWeatherOption.icon className="h-4 w-4 text-muted-foreground" />}
        ...
        <div className="space-y-4 max-h-[360px] overflow-y-auto pr-1">
          ...
          <div className="flex items-center justify-between pt-2 pb-2 border-b border-border/30">
            ...
            <Button
              ...
              size="sm"
              className={cn("h-6 text-xs px-2", focusMode && "bg-primary text-primary-foreground")}
            ...
            <div className="grid grid-cols-4 gap-1">
              {SPOTIFY_GENRES.map((genre) => (
                <Button
                  ...
                  size="sm"
                  className={cn("h-7 text-[10px] p-1", state.currentGenre === genre.id && "ring-1 ring-primary")}
            ...
            <div className="grid grid-cols-3 gap-1">
              {WEATHER_OPTIONS.map(({ mode, icon: Icon, label }) => (
                <Button
                  ...
                  size="sm"
                  className={cn("h-8 text-xs flex-col gap-0.5 p-1", weather === mode && "ring-1 ring-primary")}
            ...
            <div className="grid grid-cols-5 gap-1">
              {SEASON_OPTIONS.map(({ mode, icon: Icon, label }) => (
                <Button
                  ...
                  size="sm"
                  className={cn("h-8 text-xs flex-col gap-0.5 p-1", season === mode && "ring-1 ring-primary")}
            ...
            <p className="text-[10px] text-center text-muted-foreground">
              {focusMode
                ...
                : enabled && weather !== "clear"
                  ? `${activeWeatherOption?.label}${season !== "none" ? ` • ${activeSeasonOption?.label}` : ""} (Visual)`
                  : state.currentGenre
